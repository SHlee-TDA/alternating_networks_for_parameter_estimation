...
\begin{figure}[htbp]
    ...
    A naive direct mapping (red dashed arrow) frequently fails due to incomplete information. 
...
\end{equation}
By alternating between the hidden state and parameter updates, the estimate converges to a physically consistent fixed-point $\theta^*$ that resolves the circular dependency.
...
By conditioning on $\theta$, $H_\phi$ structurally resolves the non-injectivity of the projection $h$, estimating a full state trajectory $\mathbf{x}=[\mathbf{x}_\mathrm{obs}, \hat{\mathbf{x}}_\mathrm{hid}]^\top$ governed by the underlying dynamics.
...
\end{equation*}
Whether the resulting fixed point actually coincides with the ground truth is a separate question, which we take up in Section~\ref{sec:correctness}: there we show that the fixed point is bounded by the conditional mean and, under non-identifiability, is displaced off the physical solution manifold, so this target is in general not met.
...
\label{alg:inference}
\begin{algorithmic}[1]
    \REQUIRE observed samples $\mathbf{x}_\mathrm{obs}=[x_{\mathrm{obs}}(t_0),\ldots,x_{\mathrm{obs}}(t_{N-1})]^\top$; 

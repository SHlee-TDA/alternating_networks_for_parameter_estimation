...
\paragraph{The four noise classes.}
The training and sampling pipeline involves four distinct noise sources, and the analysis assigns each
a separate role; conflating them is what produces the apparent ``contraction vs.\ collapse'' paradox.
\emph{(N1) Condition noise}: noise added to the \emph{conditioning} input during training. By the
noise-equals-Tikhonov identity \citep{bishop1995} it penalizes the condition-Jacobian of each decoder,
promoting small $L_H,L_P$---it acts on the \emph{rate} (Section~\ref{sec:rate}).
\emph{(N2) Target noise}: noise added to the training targets; it shapes the learned conditionals
themselves and therefore enters the analysis only through the approximation defects
$\varepsilon_H,\varepsilon_P$ (Section~\ref{sec:steering}).
\emph{(N3) The CVAE latent} $\bm z$ and \emph{(N4) the inference-time injection noise} $\sigma\bm\xi$:
these set the \emph{spread} of each conditional---N4 keeps the kernel nondegenerate and powers the
minorization (Section~\ref{sec:existence})---and both \emph{cancel} in every contraction coupling
below, so neither enters $\kappa$. Remark~\ref{rem:noises} completes the argument.
...
formula); a Monte-Carlo run confirms them and demonstrates single-run estimability of $\einc$. All code
is NumPy/SciPy and reproduces in seconds; details in the supplementary notes.
...
autocorrelation, an independent check---and unequal marginal scales make one individual $L>1$ while
$\kappa<1$, illustrating Remark~\ref{rem:noises}. Figure~\ref{fig:Ea} confirms both claims: at
$\rho=0.8$ ($L_H=1.60>1$, $L_P=0.40$, $\kappa=0.640$) the exact AR(1) $\bth$-chain's $W_2$ to
stationarity decays geometrically at slope exactly $\log\kappa$, saturating the rate bound of
Theorem~\ref{thm:rate}, and the stationary law matches $\nu^\dagger$ to $10^{-10}$ (Theorem~\ref{thm:B}
exactness). The Monte-Carlo estimate tracks the closed form down to its finite-sample floor.

\begin{figure}[h]
\centering
\includegraphics[width=0.62\linewidth]{figures/num_Ea_rate.pdf}
\caption{\textbf{E-a.} Geometric convergence at rate $\kappa=\rho^2=0.640$; closed-form $W_2$ (blue)
saturates the Theorem~\ref{thm:rate} bound (red), and $\nu^\ast=\nu^\dagger$ exactly.}
\label{fig:Ea}
\end{figure}

\paragraph{(E-b) Controlled incompatibility.}
Perturbing the $P$-conditional slope by $\delta$ (keeping $\varepsilon_H=0$, dialing
$\varepsilon_P=\delta U$ on the box of radius $U$) makes the pair incompatible while keeping the chain
Gaussian. Figure~\ref{fig:Eb} shows both bounds hold across $\delta\in[0,0.18]$: the steering bound
$\varepsilon_P/(1-\kappa_\delta)$ dominates the true $W_2(\nu^\ast_\delta,\nu^\dagger)$
(Theorem~\ref{thm:B}), and the incompatibility bound $C_P\varepsilon_P$ dominates the closed-form
$\einc$ (Theorem~\ref{thm:incbound}), with $\einc/\varepsilon_P\in[0.035,0.039]$ bounded (so $\einc$
tracks $\varepsilon_P$) and $\einc(0)=0$ (compatible). A single-run empirical-covariance estimate of
$\einc$ matches the closed form ($0.109$ vs $0.105$ at $\delta=0.18$); the general nonparametric
sliced-$W_2$ estimator (used on the real model in the companion application) lower-bounds it, above its
$\delta{=}0$ resolution floor.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\linewidth]{figures/num_Eb_bounds.pdf}
\caption{\textbf{E-b.} Both theoretical bounds (red dashed) hold for every incompatibility level
$\delta$; the computable $\einc$ (blue) tracks $\varepsilon_P$ and vanishes with it. Panel B overlays a
single-run empirical-covariance Bures estimate (green) and the general sliced-$W_2$ lower bound (purple)
with its null floor.}
\label{fig:Eb}
\end{figure}
...
posterior $\{ab=x_{\mathrm{obs}}\}$---the pure-mathematics reduction of the application's
$S_I\!\cdot\!\sigma$ fiber. With learned $=$ true conditionals the experiment isolates
non-identifiability. Figure~\ref{fig:Ec} shows the stochastic dual sweep covers the ridge (HPD$95$
coverage $0.95$, sliced-$W_2$ to the grid reference $0.046$) with along-ridge spread $1.19$, while a
deterministic mean-map ping-pong collapses to a single point (along-ridge spread $0.000$); the monitored
$\einc=0.008\approx0$ certifies compatibility on the ridge (Proposition~\ref{prop:inc}). This is the
distributional analogue of the regression-to-mean collapse that motivates the whole construction.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\linewidth]{figures/num_Ec_ridge.pdf}
\caption{\textbf{E-c.} Left: the stochastic sweep (blue) covers the non-identifiable ridge $ab=1$ while
the deterministic ping-pong (red $\times$) collapses to a point. Right: along-ridge coordinate---broad
under the sampler, degenerate under the deterministic map.}
\label{fig:Ec}
\end{figure}
...
on it. Beyond the controlled examples of Section~\ref{sec:numerics}, the hypothesis and the diagnostics
it motivates are also borne out on the trained application model: an ablation isolating N1 (condition
noise) reports $\kappa$ rising from $0.406\pm0.00$ to $0.709\pm0.04$ when condition noise is removed
(as $\kappa\uparrow1$ predicts), the full model's self-consistency defect sits at
$\einc\approx0.029\pm0.00$, and the pseudo-Gibbs chain's lag-$1$ autocorrelation is $0.012$ (fast
mixing, consistent with $\kappa<1$) \citep{adcvae-application}. These are measurements on a real,
non-Gaussian trained model, not a restatement of the theorem, and the point is only that $\kappa<1$ and
small $\einc$ are not merely closed-form artifacts of Section~\ref{sec:numerics}.
...
\bibitem[Anonymous(2026)]{adcvae-application}
Anonymous.
Probabilistic decoupling for nonlinear inverse problems: overcoming model collapse in
unidentifiable systems.
\emph{Companion application manuscript, under review}, 2026.
